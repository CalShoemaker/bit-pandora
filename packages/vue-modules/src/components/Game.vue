  ...
  <div class="flex flex-col bp-game traditional" :key="uuid">
    ...
    <div class="flex flex-row h-20">
      <div class="flex flex-grow" v-if="player">
        <span class="player-icon m-3">{{
          player.emoji ? JSON.parse(player.emoji).icon : ""
        }}</span>
      </div>
      <div class="flex flex-grow justify-end" v-if="opponent">
        <span class="player-icon m-3">{{
          opponent.emoji ? JSON.parse(opponent.emoji).icon : ""
        }}</span>
      </div>
    </div>
    ...
    <template v-if="(status && status.win) || (status && status.lose)">
      <div class="modal">
        <div class="text-white">
          <h2>
            Game Over {{ player.name || "booo" }}. Player:
            {{ status.finish }} won!
          </h2>
          <ul>
...
export default defineComponent({
  ...
  computed: {
    ...
    history() {
      return this.pid && this.players && this.players[this.pid]
        ? $(this.players[this.pid]).games.current.history
        : [];
    },
    tiles() {
      return this.pid && this.players && this.players[this.pid]
        ? $(this.players[this.pid]).games.current.tiles
        : [];
    },
    canPlay() {
      return this.pid && this.players && this.players[this.pid]
        ? $(this.players[this.pid]).games.current.canPlay
        : [];
    },
    opponent() {
  ...
  methods: {
    ...
    joinGame() {
      this.newPlayer({ name: this.guest.name, emoji: this.guest.emoji }).then(
        (UserPlayer) => {
