<template>
  <div
    class="grid grid-cols-3 facet"
    :class="{ enabled: canPlay.length > 0 }"
    v-if="pid"
  >
    <div
      v-for="(number, index) in range"
      v-bind:key="index"
      class="flip-card"
      :class="{
        ...
        closed:
          opponent &&
          opponent.games.current &&
          !opponent.games.current.tiles.includes(number),
      }"
    >
      <div
        class="flip-card-inner flex items-center justify-center"
        v-touch:longtap="click('give', number)"
        v-touch="click('take', number)"
...
export default defineComponent({
  ...
  computed: {
    ...mapGetters("pandoraModule", ["players", "status"]),
  },
  methods: {
    ...mapActions("pandoraModule", ["Pick"]),
    pointerDown(e: Event) {
    ...
    canPick(conditions: Conditions) {
      ...
      const sum = (a: Array<number>, s: number) =>
        a.reduce((p: number, a: number) => p + a, s);
      ...
      const state = {
        ...
        max: function () {
          return Math.max(...this.flats.c, this.sums.l);
        },
      };

      if (state.sums.s > state.max()) {
        this.selected = [];
      }
      if (state.sums.g > state.max()) {
        this.given = [];
      }

      return (
        (state.sums.s === state.max() && state.flats.c.includes(n)) ||
        state.sums.g === state.max() ||
        state.sums.g + state.sums.s === state.max()
      );
    },
