...
export class GameService {
  ...
  public getAll = () => {
    return { history: this.history, players: this.players };
  };

  // TODO: (P4) Improve randomish die.
  public die = (min: number, max: number) =>
    min + Math.floor(Math.random() * (max - min + 1));

  // NOTE: Roll has implicit game logic as only 1 or 2 dice can be rolled at a time.
  public roll = (i: number) =>
    i === 1 ? [this.die(1, 6)] : [this.die(1, 6), this.die(1, 6)];
        ...
        if (max > 1) {
          for (const [key, value] of Object.entries(players)) {
            if (key !== player.pid.toString()) {
              winner =
                players[key] && players[key].name ? players[key].name : winner;
            }
          }
        }
        status.lose = true;
  ...
  public Cast(pid: number, id: number, d: number) {
    ...
    if (!pid || !id || !d)
      return { message: 'Player or ID or Number missing.' };
    ...
    if (canPlay.length !== 0)
      return { message: 'Player has avilable options.', player, id, canPlay };
  ...
  public Pick(pid: number, id: number, solution: any) {
    ...
    if (!pid || !id || !solution)
      return { message: 'Player, ID, or Solution is missing,' };
    ...
    const inCanPlay = canPlay.filter((a: Array<number>) =>
      checkSubset(a, merged),
    );
    ...
    if (inTiles && inCanPlay) {
      ...
      if (victim && give) {
        give.forEach((n) => {
          if (!victim.games.current.tiles.includes(n))
            victim.games.current.tiles.push(n);
        });
      }

      player.games.current.canPlay = [];

      let tiles = player.games.current.tiles.filter(
        (tile) => !merged.includes(tile),
      );
  ...
  solutionSpace(arr: Array<number>, target: number) {
    ...
    const removeDuplicates = (ar: Array<number>) =>
      ar.filter((n, i) => ar.indexOf(n) === i);
    let undupe = [];
    ...
    let dedupe = undupe.filter((set) =>
      set.length > 1 ? set[0] !== set[1] : true,
    );

    // Remove remainder sets that don't sum to total
    let reduce = dedupe.filter(
      (set) => set.reduce((p, a) => p + a, 0) === target,
    );
